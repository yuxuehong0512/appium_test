...
def test():
    ...
    setattr(Context, "book_name", book_name)
    setattr(Context, "book_desc", book_desc)
    setattr(Context, "book_chapter", book_chapter)
    setattr(Context, "book_author", book_author)


def test_002():
    driver = getattr(Context, "driver")
    book_name = getattr(Context, "book_name")
    ...
    Wait_visibity(driver, *lacator).click()
    print("这里在验证搜索功能的完整性")
...
# 获取所有的章节目录
    driver = getattr(Context, "driver")
    book_name = getattr(Context, "book_name")
    book_desc = getattr(Context, "book_desc")
    book_author = getattr(Context, "book_author")
    book_chapter = getattr(Context, "book_chapter")
    ...
    if search_index is None:
        ...
        # 将目录中章节顺序和章节内容存储在字典中
        locator = (
            AppiumBy.XPATH, '//*[@resource-id="com.zhao.myreader:id/lv_chapter_list"]/android.widget.LinearLayout')
