...
class PageObject:
    ...
    def search_action(self, search_contant):

        """
        搜索这个动作
        :return:
        """
        wait_visibility(self.driver, *self.Input.search_key).send_keys(search_contant) #搜索框输入
        wait_visibility(self.driver, *self.Button.search_conform).click() #点击搜索
        return self
